...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , rst_driver("rst_driver", ac_env::read_float("SCVerify_RESET_CYCLES")*1.110000, false)
    , TLS_rst("TLS_rst")
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(env_SCVerify_RESET_CYCLES*1.110000, SC_NS);
  } else {
    if (toggle_hw_reset) {
