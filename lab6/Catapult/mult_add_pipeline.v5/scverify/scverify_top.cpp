...
public:
  ...
  scverify_top(const sc_module_name& name)
    ...
    , CCS_CLK_CTOR(clk, "clk", 9.09, SC_NS, 0.5, 0, SC_NS, false)
    , rst_driver("rst_driver", ac_env::read_float("SCVerify_RESET_CYCLES")*9.090000, false)
    , TLS_rst("TLS_rst")
...
void scverify_top::generate_reset() {
  ...
  if (activate_reset || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    generate_reset_event.notify(env_SCVerify_RESET_CYCLES*9.090000, SC_NS);
  } else {
    if (toggle_hw_reset) {
...
void scverify_top::deadlock_watch() {
  ...
  if (!clk) {
    if (rst == SC_LOGIC_0 &&
      ...
      deadlock_event.notify(280, SC_NS);
    } else {
