module booth (
    ...
    logic        [31:0] result_r;
    logic        [31:0] result_n;
    logic        [31:0] operand;     // Input to the adder
    logic        [31:0] data_a_comp; // data_a's compliment (data_a = 12 -> data_a_comp = -12)
    logic        [31:0] data_a_ext;  // data_a sign extended to 32 bits
    logic        [16:0] data_b_pad;  // data_b with an additional 0 for the last window
    logic        [ 2:0] win_cnt_r;
    logic        [ 2:0] win_cnt_n;
    logic        [ 2:0] action;
    logic               busy_n;
    ...
    always_comb begin : set_action
        ...
        if (start_posedge) begin
            ...
            busy_n   = 1'b1;
            win_cnt_n++;
        end else if (|win_cnt_n) begin
            result_n = result_n << 2;
            win_cnt_n++;
        end else begin
            operand  = '0; // Set to 0 to ensure result stays the same
        end

        // Perform addition in 32-bit width
        result_n = operand + result_n;

        if (ack && (~|win_cnt_n)) begin
            irq_n  = 1'b0;
            busy_n = 1'b0;
        end
    end
